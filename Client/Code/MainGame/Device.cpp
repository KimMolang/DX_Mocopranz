...
HRESULT CDevice::Init()
{
	...
	tSwapChain.BufferDesc.Width = (UINT)800;	// (荐沥) (可记)
	tSwapChain.BufferDesc.Height = (UINT)600;
	...
	tSwapChain.Windowed = true;		// (荐沥) (可记)
	...
	CHECK_FAILED(m_pSwapChain->ResizeBuffers(
		1, (UINT)800, (UINT)600, DXGI_FORMAT_R8G8B8A8_UNORM, 0));
	// (荐沥) (可记)
	...
	tDepthStencil.Width = (UINT)800;		// (荐沥) (可记)
	tDepthStencil.Height = (UINT)600;
